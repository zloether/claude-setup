...
5. **Set up project-level settings**: remind the user that each project needs a `.claude/settings.json` to allow Read/Edit within the project directory (see README for the template). Claude Code only checks file permissions against `Read(path)` and `Edit(path)` rules — an `Edit(./**)` rule also covers the Write tool, so a `Write(./**)` rule is unnecessary. Without a project settings file, Claude cannot read or write project files at all — `dontAsk` mode denies everything not explicitly allowed. Always use scoped patterns like `"Read(./**)"` instead of bare `"Read"` — bare patterns allow reads across the entire filesystem. This repo's own `.claude/settings.json` is an example of the scoped form.
...
- Sandbox enabled with OS-level `denyRead` for credential paths (`~/.aws`, `~/.ssh`, `~/.gnupg`, and targeted `~/.config` subdirs: `gcloud`, `op`, `doctl`, `fly`, `stripe`, `heroku`, `pypoetry`, `configstore`, `helm`, `rclone`, `restic`) — note the sandbox applies to shell (Bash) commands only, not to Claude's native Read/Edit/Write tools, which are governed by permission rules. Important: the sandbox automatically merges `permissions.deny` Read rules into its OS-level enforcement, so a path in `permissions.deny` also blocks subprocess reads — there is no way to block Claude's Read tool while leaving a subprocess free to read the same path. CLI tools that need to authenticate (`gh`, `netlify`, `vercel`) are omitted from both lists so their subprocesses can read credentials normally.
